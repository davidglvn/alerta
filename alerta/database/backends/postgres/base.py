...
class Backend(Database):
        ...
        update = """
            UPDATE alerts
               SET status=%(status)s, value=%(value)s, text=%(text)s, timeout=%(timeout)s, raw_data=%(raw_data)s,
                   ...
                   duplicate_count=duplicate_count+1, history=(history || %(history)s)[array_length(history,1)-{limit}+2:array_length(history,1)+1]
             WHERE environment=%(environment)s
               AND resource=%(resource)s
               AND event=%(event)s
               AND severity=%(severity)s
        ...
        update = """
            UPDATE alerts
               SET event=%(event)s, severity=%(severity)s, status=%(status)s, value=%(value)s, text=%(text)s,
                   ...
                   attributes=attributes || %(attributes)s, history=(history || %(history)s)[array_length(history,1)-{limit}+2:array_length(history,1)+1]
             WHERE environment=%(environment)s
               AND resource=%(resource)s
    ...
    def set_status(self, id, status, timeout, history=None):
        update = """
            UPDATE alerts
            SET status=%(status)s, timeout=%(timeout)s, history=(history || %(change)s)[array_length(history,1)-{limit}+2:array_length(history,1)+1]
    ...
    def set_severity_and_status(self, id, severity, status, timeout, history=None):
        update = """
            UPDATE alerts
            SET severity=%(severity)s, status=%(status)s, timeout=%(timeout)s, history=(history || %(change)s)[array_length(history,1)-{limit}+2:array_length(history,1)+1]
            WHERE id=%(id)s OR id LIKE %(like_id)s
            RETURNING *
        """.format(limit=current_app.config['HISTORY_LIMIT'])
